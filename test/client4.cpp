/* 业务处理超时，查看服务器的处理情况
    当服务器达到了一个性能瓶颈，在一次业务处理中花费了太长的时间（超过了服务器设置的非活跃超时时间）
     ...
     假设现在  1,2,3,4,5描述符就绪了， 在处理1的时候花费了30s处理完，超时了，导致2,3,4,5描述符因为长时间没有刷新活跃度
       1. 如果接下来的2345描述符都是通信连接描述符，如果都就绪了，则并不影响，因为接下来就会进行处理并刷新活跃度
       2. 如果接下来的2号描述符是定时器事件描述符，定时器触发超时，执行定时任务，就会将3,4,5描述符给释放掉
          这时候一旦3,4,5描述符对应的连接被释放，接下来在处理3,4,5事件的时候就会导致程序崩溃（内存访问错误）
