...
class LoopThread
{
    ...
    public:
        ...
        EventLoop *GetLoop()
        {
            ...
            {
               std::unique_lock<std::mutex> _lock(_mutex);
               _cond.wait(_lock, [&](){return (_loop != NULL);});//loop为NULL就一直阻塞
               loop = _loop; 
            }
            return loop;
        }
...
{
    private:
        int _thread_count;                  // 线程池中线程的数量
        int _next_idx;                      // 下一个要使用的线程索引
        EventLoop *_baseloop;               // 基础 EventLoop 对象
        std::vector<LoopThread*> _threads;  // 线程池中的线程对象
        std::vector<EventLoop*> _loops;     // 线程池中每个线程对应的 EventLoop 对象
        ...
        void Create()
        {
            if(_thread_count > 0)
            {
                ...
                for(int i = 0; i < _thread_count; i++)
                {
                    ...
                    _loops[i] = _threads[i]->GetLoop();//new完线程后，才能用，不然会阻塞在这
                    //GetLoop是获取对应的 EventLoop 对象
                    //每个 LoopThread 对象在创建时会自动启动一个线程，并运行 ThreadEntry 方法
                }
            }
            return;
        }

        //返回下一个可用的 EventLoop 对象。
        EventLoop *NextLoop()
        {
            if(_thread_count == 0)
            {
...
class Any
{
    ...
    public:
        ...
        // 模板构造函数，接受任意类型的数据，创建一个 placeholder 对象存储数据
        
        Any(const Any &other):_content(other._content ? other._content->clone() : NULL){}
        // 拷贝构造函数，通过 clone() 方法克隆另一个 Any 对象的内容
        ~Any(){delete _content;}
...
class Connection : public std::enable_shared_from_this<Connection>
{
    private:
        ...
        //四个回调函数，是让服务器模块来设置的（其实服务器模块的处理回调也是组件使用者设置的）
        ...
        MessageCallback _message_callback;//消息处理回调
    ...
    private:
        ...
        void HandleWrite()
        {
            ...
            if(_out_buffer.ReadAbleSize() == 0)
            {
                ...
                //如果当前是连接待关闭状态，并且有数据，发送完数据释放连接，没有数据则直接释放
    ...
    public:
        ...
        //切换协议---重置上下文以及阶段性回调处理函数 -- 这个接口必须在EventLoop线程中立即执行
...
{
    private:
        uint64_t _next_id;                                  //连接和定时任务用到的id,自动增长
        int _port;
